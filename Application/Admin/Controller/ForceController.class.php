...
class ForceController extends CommonController {
   ...
   public function index(){
	   ...
	   // $data=$this->pagelink('admin_Force','','id asc',10);
	   // $show = $data['show'];
	   // $list = $data['list'];
	   ...
	   if(IS_POST){
		   ...
		   $shouye=M('admin_Force');
		   $row = $shouye->delete($idarr);
		   if($row){
				alert('删除成功',U('Force/index'));
		   }else{
				alert('删除失败');
		   }
	   }
