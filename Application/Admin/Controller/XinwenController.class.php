...
class XinwenController extends CommonController {
   ...
   public function class_list(){
	   ...
	   //var_dump($row);die;*/
	   $data=$this->pagelink('admin_xinwen','','id desc',20);
	   $show = $data['show'];
	   $list = $data['list'];
	   ...
	   if(IS_POST){
		   ...
		   $shouye=M('admin_xinwen');
		   $row = $shouye->delete($idarr);
		   if($row){
				alert('删除成功',U('Xinwen/class_list'));
		   }else{
				alert('删除失败');
		   }
	   }
   ...
   public function add(){
	   if(isset($_POST["dosubmit"])){
		...
		if($_FILES['tupian']['error']==0){
		   ...
		   $row = $user -> add($data);
		   if($row){
			   alert('插入成功',U('Xinwen/class_list'));
		   }else{
			   $this->error("插入失败！");
		   }
	   }
	...
	if($_FILES['tupian']['error']==0){
	 $upload = new \Think\Upload();                                    // 实例化上传类   
		...
		$sql = $user->where("id='$id'")->save($data);
		 if($sql){
		   alert('更改成功',U('Xinwen/class_list'));
	   }else{
		   $this->error("更改失败！");
	   }
		  }
