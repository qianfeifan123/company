   ...
   <div class="row">
      ...
      <div class="panel">
                <div class="panel-heading">
                 ...
                 <!--  <a href="<?php echo U("Service/add"); ?>" class="btn btn-info btn-gradient pull-right"><span class="glyphicons glyphicon-plus"></span> 添加文章</a> -->
                 <p class="pull-right"><input type="input" name="search" value="" placeholder="请输入标题"><input type="submit" value="搜索" style="width:100px;" onclick="search()"></p>
                </div>
                <form action="" method="post">
                <div class="panel-body">
                  <h2 class="panel-body-title">服务中心</h2>
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr class="success">
                          <td class="text-center"><input type="checkbox" value="<?php echo ($list["id"]); ?>" name="idarr[]" class="cbox"></td>
                          <td><?php echo ($list["id"]); ?></td>
                          <td><?php echo ($list["title"]); ?></td>
                          <td><?php echo str_cut($list['content'],0,20,'utf8','...')?></td>
                          <!-- <td><img src="/erongchuang/Uploads/<?php echo ($list["img"]); ?>" width="100px" height="100px"></td> -->
                          <!-- <td><?php echo date("Y-m-d h:i:s",$list['times']);?></td> -->
                          <td>
                            <div class="btn-group">
                              <a href="<?php echo U('Service/edit',array('id'=>$list['id']));?>" class="btn btn-default btn-gradient"><span class="glyphicons glyphicon-pencil"></span></a>
                              <a onclick="return confirm('确定要删除吗？');" href=" <?php echo U('Service/del',array('id'=>$list['id']));?>" class="btn btn-default btn-gradient dropdown-toggle"><span class="glyphicons glyphicon-trash"></span></a>
                            </div>
                          
                          </td>
                </tr>
                <?php $_result=pidType($list['id']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr class="success">
                          <td class="text-center"><input type="checkbox" list="<?php echo ($list["id"]); ?>" name="idarr[]" class="cbox"></td>
                          <td><?php echo ($list["id"]); ?></td>
                          <td>--<?php echo ($list["title"]); ?></td>
                          <td>--<?php echo str_cut($list['content'],0,20,'utf8','...')?></td>
                          <!-- <td><img src="/erongchuang/Uploads/<?php echo ($list["img"]); ?>" width="100px" height="100px"></td> -->
...
function search(){
  ...
  var url=url.substring(0,url.length - 5);
